...
using DepartmentApp.Authentication;
using DepartmentApp.Entities;
using DepartmentApp.EntityFrameworkCore;

namespace DepartmentApp.Web.Host.Controllers
{
        ...
        public ActionResult Authenticate([FromBody] AuthenticateModel model)
        {
            try
            {
                ...
                if (user.PasswordHash != null && user.PasswordHash.StartsWith("PBKDF2$"))
                    pwOk = DepartmentApp.Users.PasswordHelper.VerifyPbkdf2(model.Password, user.PasswordHash);
                else
