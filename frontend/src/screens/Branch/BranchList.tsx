...
export const BranchList: React.FC = () => {
  const list = useListQuery<BranchRecord>({ resource: 'Branch' });
  const [showCreate, setShowCreate] = useState(false);
  const [editRecord, setEditRecord] = useState<BranchRecord | null>(null);
  const [selectedRows, setSelectedRows] = useState<BranchRecord[]>([]);
  const { triggerFlows } = useFlows();
  const del = useDeleteMutation('Branch', (ids) => { ids.forEach(id => triggerFlows('delete', 'Branch', { id })); });
  ...
  return (
    <>
      <ListPageLayout
        ...
        onSelectionChange={(rows) => setSelectedRows(rows as BranchRecord[])}
        ...
        headerActions={<>
          ...
          <TkButton label="+ Create Branch" variant="primary" onTkClick={() => setShowCreate(true)} />
        </>}
      />
      ...
      />
      <BranchCreate open={showCreate} onClose={() => setShowCreate(false)} onSuccess={list.invalidate} />
      <BranchEdit record={editRecord} onClose={() => setEditRecord(null)} onSuccess={list.invalidate} />
