...
import { OvertimeRecordCreate } from './OvertimeRecordCreate';
import { OvertimeRecordEdit } from './OvertimeRecordEdit';
...
const COLUMNS: TableColumn[] = [
  ...
  { field: 'overtimeDate', header: 'Overtime Date', sortable: true, filterType: 'datepicker', html: (row: Record<string, unknown>) => row.overtimeDate ? new Date(String(row.overtimeDate)).toLocaleDateString() : '—' },
  { field: 'hours', header: 'Hours', sortable: true },
  { field: 'reason', header: 'Reason', sortable: true, searchable: true, filterType: 'text' },
  { field: 'status', header: 'Status', sortable: true, filterType: 'radio', filterOptions: [{ label: 'Pending', value: '0' }, { label: 'Approved', value: '1' }, { label: 'Rejected', value: '2' }, { label: 'Cancelled', value: '3' }], html: (row: Record<string, unknown>) => { const m: Record<string, string> = {'0': 'Pending', '1': 'Approved', '2': 'Rejected', '3': 'Cancelled'}; return m[String(row.status ?? '')] ?? String(row.status ?? '\u2014'); } },
  { field: 'approverNote', header: 'Approver Note', sortable: true, searchable: true, filterType: 'text' },
...
export const OvertimeRecordList: React.FC = () => {
  const list = useListQuery<OvertimeRecordRecord>({ resource: 'OvertimeRecord' });
  const [showCreate, setShowCreate] = useState(false);
  const [editRecord, setEditRecord] = useState<OvertimeRecordRecord | null>(null);
  const [selectedRows, setSelectedRows] = useState<OvertimeRecordRecord[]>([]);
  const { triggerFlows } = useFlows();
  const del = useDeleteMutation('OvertimeRecord', (ids) => { ids.forEach(id => triggerFlows('delete', 'OvertimeRecord', { id })); });
  ...
  return (
    <>
      <ListPageLayout
        ...
        onSelectionChange={(rows) => setSelectedRows(rows as OvertimeRecordRecord[])}
        ...
        headerActions={<>
          ...
          <TkButton label="+ Create OvertimeRecord" variant="primary" onTkClick={() => setShowCreate(true)} />
        </>}
      />
      ...
      />
      <OvertimeRecordCreate open={showCreate} onClose={() => setShowCreate(false)} onSuccess={list.invalidate} />
      <OvertimeRecordEdit record={editRecord} onClose={() => setEditRecord(null)} onSuccess={list.invalidate} />
