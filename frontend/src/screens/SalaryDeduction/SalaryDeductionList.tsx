...
import { SalaryDeductionCreate } from './SalaryDeductionCreate';
import { SalaryDeductionEdit } from './SalaryDeductionEdit';
...
const COLUMNS: TableColumn[] = [
  ...
  { field: 'deductionType', header: 'Deduction Type', sortable: true, searchable: true, filterType: 'text' },
  ...
  { field: 'salaryRecordId', header: 'Maaş Kaydı', sortable: true },
];
...
export const SalaryDeductionList: React.FC = () => {
  const list = useListQuery<SalaryDeductionRecord>({ resource: 'SalaryDeduction' });
  const [showCreate, setShowCreate] = useState(false);
  const [editRecord, setEditRecord] = useState<SalaryDeductionRecord | null>(null);
  const [selectedRows, setSelectedRows] = useState<SalaryDeductionRecord[]>([]);
  const { triggerFlows } = useFlows();
  const del = useDeleteMutation('SalaryDeduction', (ids) => { ids.forEach(id => triggerFlows('delete', 'SalaryDeduction', { id })); });
  ...
  return (
    <>
      <ListPageLayout
        ...
        onSelectionChange={(rows) => setSelectedRows(rows as SalaryDeductionRecord[])}
        ...
        headerActions={<>
          ...
          <TkButton label="+ Create SalaryDeduction" variant="primary" onTkClick={() => setShowCreate(true)} />
        </>}
      />
      ...
      />
      <SalaryDeductionCreate open={showCreate} onClose={() => setShowCreate(false)} onSuccess={list.invalidate} />
      <SalaryDeductionEdit record={editRecord} onClose={() => setEditRecord(null)} onSuccess={list.invalidate} />
