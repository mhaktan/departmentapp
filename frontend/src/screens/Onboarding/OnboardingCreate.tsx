...
export const OnboardingCreate: React.FC<OnboardingCreateProps> = ({ open, onClose, onSuccess }) => {
  ...
  const mutation = useMutation({
    mutationFn: (values: Partial<OnboardingRecord>) => dataProvider.create('Onboarding', values),
    onSuccess: (_data, values) => { triggerFlows('create', 'Onboarding', values as Record<string, unknown>); onSuccess(); onClose(); setForm({}); },
    ...
    <div style={overlayStyle} onClick={onClose}>
      <div style={modalStyle} onClick={(e) => e.stopPropagation()}>
        <div style={{ padding: '20px 28px 0', flexShrink: 0, display: 'flex', justifyContent: 'space-between', alignItems: 'center' }}>
          <h2 style={{ margin: 0, fontSize: 18, fontWeight: 700 }}>Create Onboarding</h2>
