...
import { SalaryRecordCreate } from './SalaryRecordCreate';
import { SalaryRecordEdit } from './SalaryRecordEdit';
...
const COLUMNS: TableColumn[] = [
  ...
  { field: 'salaryType', header: 'Salary Type', sortable: true, filterType: 'radio', filterOptions: [{ label: 'Monthly', value: '0' }, { label: 'Hourly', value: '1' }, { label: 'Daily', value: '2' }], html: (row: Record<string, unknown>) => { const m: Record<string, string> = {'0': 'Monthly', '1': 'Hourly', '2': 'Daily'}; return m[String(row.salaryType ?? '')] ?? String(row.salaryType ?? '\u2014'); } },
...
export const SalaryRecordList: React.FC = () => {
  const list = useListQuery<SalaryRecordRecord>({ resource: 'SalaryRecord' });
  const [showCreate, setShowCreate] = useState(false);
  const [editRecord, setEditRecord] = useState<SalaryRecordRecord | null>(null);
  const [selectedRows, setSelectedRows] = useState<SalaryRecordRecord[]>([]);
  const { triggerFlows } = useFlows();
  const del = useDeleteMutation('SalaryRecord', (ids) => { ids.forEach(id => triggerFlows('delete', 'SalaryRecord', { id })); });
  ...
  return (
    <>
      <ListPageLayout
        ...
        onSelectionChange={(rows) => setSelectedRows(rows as SalaryRecordRecord[])}
        ...
        headerActions={<>
          ...
          <TkButton label="+ Create SalaryRecord" variant="primary" onTkClick={() => setShowCreate(true)} />
        </>}
      />
      ...
      />
      <SalaryRecordCreate open={showCreate} onClose={() => setShowCreate(false)} onSuccess={list.invalidate} />
      <SalaryRecordEdit record={editRecord} onClose={() => setEditRecord(null)} onSuccess={list.invalidate} />
