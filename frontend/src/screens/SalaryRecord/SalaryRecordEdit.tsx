...
interface SalaryRecordEditProps {
  record: SalaryRecordRecord | null;
  onClose: () => void;
  onSuccess: () => void;
}

export const SalaryRecordEdit: React.FC<SalaryRecordEditProps> = ({ record, onClose, onSuccess }) => {
  const [form, setForm] = useState<Partial<SalaryRecordRecord>>(record ?? {});
  ...
  const mutation = useMutation({
    mutationFn: (values: Partial<SalaryRecordRecord>) =>
      dataProvider.update('SalaryRecord', record!.id, values),
    onSuccess: (_data, values) => { triggerFlows('update', 'SalaryRecord', values as Record<string, unknown>); onSuccess(); onClose(); },
    ...
    <div style={overlayStyle} onClick={onClose}>
      <div style={modalStyle} onClick={(e) => e.stopPropagation()}>
        <div style={{ padding: '20px 28px 0', flexShrink: 0, display: 'flex', justifyContent: 'space-between', alignItems: 'center' }}>
          <h2 style={{ margin: 0, fontSize: 18, fontWeight: 700 }}>Edit SalaryRecord</h2>
        ...
        <form onSubmit={(e) => { e.preventDefault(); mutation.mutate(form); }} style={{ display: 'flex', flexDirection: 'column', flex: 1, overflow: 'hidden' }}>
          <div style={{ flex: 1, overflowY: 'auto', padding: '20px 28px' }}>
            <div style={{ display: 'grid', gridTemplateColumns: '1fr 1fr', gap: '16px' }}>
                ...
                <div>
                  <TkInput mode="number" label="Gross Salary *" value={String(form.grossSalary ?? '')} onTkChange={(e: CustomEvent) => ((v) => setField('grossSalary', Number(v)))(e.detail)} />
                </div>
                <div>
                  <TkInput mode="number" label="Net Salary" value={String(form.netSalary ?? '')} onTkChange={(e: CustomEvent) => ((v) => setField('netSalary', Number(v)))(e.detail)} />
                </div>
                <div>
                  <TkInput mode="text" label="Currency *" value={String(form.currency ?? '')} onTkChange={(e: CustomEvent) => ((v) => setField('currency', v))(e.detail)} />
                </div>
                <div>
                  <LookupSelect label="Salary Type *" value={String(form.salaryType ?? '')} onChange={(v) => setField('salaryType', v ? Number(v) : null)} searchable={false} options={[{ label: 'Monthly', value: '0' }, { label: 'Hourly', value: '1' }, { label: 'Daily', value: '2' }]} />
                </div>
                <div>
