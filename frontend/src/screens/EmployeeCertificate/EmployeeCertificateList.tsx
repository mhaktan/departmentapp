...
import { EmployeeCertificateCreate } from './EmployeeCertificateCreate';
...
type EmployeeCertificateRecord = {
  id: string | number;
...
const COLUMNS: TableColumn[] = [
  ...
  { field: 'certificateName', header: 'Certificate Name', sortable: true, searchable: true, filterType: 'text' },
  { field: 'issuingBody', header: 'Issuing Body', sortable: true, searchable: true, filterType: 'text' },
  { field: 'issueDate', header: 'Issue Date', sortable: true, filterType: 'datepicker', html: (row: Record<string, unknown>) => row.issueDate ? new Date(String(row.issueDate)).toLocaleDateString() : '—' },
  { field: 'expiryDate', header: 'Expiry Date', sortable: true, filterType: 'datepicker', html: (row: Record<string, unknown>) => row.expiryDate ? new Date(String(row.expiryDate)).toLocaleDateString() : '—' },
  { field: 'certificateNumber', header: 'Certificate Number', sortable: true, searchable: true, filterType: 'text' },
...
export const EmployeeCertificateList: React.FC = () => {
  const list = useListQuery<EmployeeCertificateRecord>({ resource: 'EmployeeCertificate' });
  const [showCreate, setShowCreate] = useState(false);
  const [editRecord, setEditRecord] = useState<EmployeeCertificateRecord | null>(null);
  const [selectedRows, setSelectedRows] = useState<EmployeeCertificateRecord[]>([]);
  const { triggerFlows } = useFlows();
  const del = useDeleteMutation('EmployeeCertificate', (ids) => { ids.forEach(id => triggerFlows('delete', 'EmployeeCertificate', { id })); });
  ...
  return (
    <>
      <ListPageLayout
        ...
        onSelectionChange={(rows) => setSelectedRows(rows as EmployeeCertificateRecord[])}
        ...
        headerActions={<>
          ...
          <TkButton label="+ Create EmployeeCertificate" variant="primary" onTkClick={() => setShowCreate(true)} />
        </>}
      />
      ...
      />
      <EmployeeCertificateCreate open={showCreate} onClose={() => setShowCreate(false)} onSuccess={list.invalidate} />
      <EmployeeCertificateEdit record={editRecord} onClose={() => setEditRecord(null)} onSuccess={list.invalidate} />
