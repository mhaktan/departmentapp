...
interface EmployeeCertificateEditProps {
  record: EmployeeCertificateRecord | null;
  onClose: () => void;
  onSuccess: () => void;
}

export const EmployeeCertificateEdit: React.FC<EmployeeCertificateEditProps> = ({ record, onClose, onSuccess }) => {
  const [form, setForm] = useState<Partial<EmployeeCertificateRecord>>(record ?? {});
  ...
  const mutation = useMutation({
    mutationFn: (values: Partial<EmployeeCertificateRecord>) =>
      dataProvider.update('EmployeeCertificate', record!.id, values),
    onSuccess: (_data, values) => { triggerFlows('update', 'EmployeeCertificate', values as Record<string, unknown>); onSuccess(); onClose(); },
    ...
    <div style={overlayStyle} onClick={onClose}>
      <div style={modalStyle} onClick={(e) => e.stopPropagation()}>
        <div style={{ padding: '20px 28px 0', flexShrink: 0, display: 'flex', justifyContent: 'space-between', alignItems: 'center' }}>
          <h2 style={{ margin: 0, fontSize: 18, fontWeight: 700 }}>Edit EmployeeCertificate</h2>
        ...
        <form onSubmit={(e) => { e.preventDefault(); mutation.mutate(form); }} style={{ display: 'flex', flexDirection: 'column', flex: 1, overflow: 'hidden' }}>
          <div style={{ flex: 1, overflowY: 'auto', padding: '20px 28px' }}>
            <div style={{ display: 'grid', gridTemplateColumns: '1fr 1fr', gap: '16px' }}>
                <div>
                  <TkInput mode="text" label="Certificate Name *" value={String(form.certificateName ?? '')} onTkChange={(e: CustomEvent) => ((v) => setField('certificateName', v))(e.detail)} />
                </div>
                <div>
                  <TkInput mode="text" label="Issuing Body" value={String(form.issuingBody ?? '')} onTkChange={(e: CustomEvent) => ((v) => setField('issuingBody', v))(e.detail)} />
                </div>
                <div>
                  <TkDatepicker label="Issue Date" value={String(form.issueDate ?? '')} onTkChange={(e: CustomEvent) => ((v) => setField('issueDate', v))(e.detail)} />
                </div>
                <div>
                  <TkDatepicker label="Expiry Date" value={String(form.expiryDate ?? '')} onTkChange={(e: CustomEvent) => ((v) => setField('expiryDate', v))(e.detail)} />
                </div>
                <div>
                  <TkInput mode="text" label="Certificate Number" value={String(form.certificateNumber ?? '')} onTkChange={(e: CustomEvent) => ((v) => setField('certificateNumber', v))(e.detail)} />
                </div>
                <div>
