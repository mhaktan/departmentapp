...
interface LeaveTypeEditProps {
  record: LeaveTypeRecord | null;
  onClose: () => void;
  onSuccess: () => void;
}

export const LeaveTypeEdit: React.FC<LeaveTypeEditProps> = ({ record, onClose, onSuccess }) => {
  const [form, setForm] = useState<Partial<LeaveTypeRecord>>(record ?? {});
  ...
  const mutation = useMutation({
    mutationFn: (values: Partial<LeaveTypeRecord>) =>
      dataProvider.update('LeaveType', record!.id, values),
    onSuccess: (_data, values) => { triggerFlows('update', 'LeaveType', values as Record<string, unknown>); onSuccess(); onClose(); },
    ...
    <div style={overlayStyle} onClick={onClose}>
      <div style={modalStyle} onClick={(e) => e.stopPropagation()}>
        <div style={{ padding: '20px 28px 0', flexShrink: 0, display: 'flex', justifyContent: 'space-between', alignItems: 'center' }}>
          <h2 style={{ margin: 0, fontSize: 18, fontWeight: 700 }}>Edit LeaveType</h2>
        ...
        <form onSubmit={(e) => { e.preventDefault(); mutation.mutate(form); }} style={{ display: 'flex', flexDirection: 'column', flex: 1, overflow: 'hidden' }}>
          <div style={{ flex: 1, overflowY: 'auto', padding: '20px 28px' }}>
            <div style={{ display: 'grid', gridTemplateColumns: '1fr 1fr', gap: '16px' }}>
                ...
                <div>
                  <label style={{ display: 'flex', alignItems: 'center', gap: 8, fontSize: 14 }}>
                    <input type="checkbox" checked={!!form.isPaid} onChange={(e) => ((v) => setField('isPaid', v))(e.target.checked)} style={{ width: 16, height: 16 }} />
                    Is Paid *
                  </label>
                </div>
                <div>
                  <TkInput mode="number" label="Max Days Per Year" value={String(form.maxDaysPerYear ?? '')} onTkChange={(e: CustomEvent) => ((v) => setField('maxDaysPerYear', Number(v)))(e.detail)} />
                </div>
                <div>
