...
const COLUMNS: TableColumn[] = [
  ...
  { field: 'isPaid', header: 'Is Paid', sortable: true, filterType: 'checkbox', filterOptions: [{ label: 'Yes', value: 'true' }, { label: 'No', value: 'false' }], html: (row: Record<string, unknown>) => row.isPaid ? '<span style="color:#2e7d32;font-weight:600">Yes</span>' : '<span style="color:#999">No</span>' },
  { field: 'maxDaysPerYear', header: 'Max Days Per Year', sortable: true },
...
export const LeaveTypeList: React.FC = () => {
  const list = useListQuery<LeaveTypeRecord>({ resource: 'LeaveType' });
  const [showCreate, setShowCreate] = useState(false);
  const [editRecord, setEditRecord] = useState<LeaveTypeRecord | null>(null);
  const [selectedRows, setSelectedRows] = useState<LeaveTypeRecord[]>([]);
  const { triggerFlows } = useFlows();
  const del = useDeleteMutation('LeaveType', (ids) => { ids.forEach(id => triggerFlows('delete', 'LeaveType', { id })); });
  ...
  return (
    <>
      <ListPageLayout
        ...
        onSelectionChange={(rows) => setSelectedRows(rows as LeaveTypeRecord[])}
        ...
        headerActions={<>
          ...
          <TkButton label="+ Create LeaveType" variant="primary" onTkClick={() => setShowCreate(true)} />
        </>}
      />
      ...
      />
      <LeaveTypeCreate open={showCreate} onClose={() => setShowCreate(false)} onSuccess={list.invalidate} />
      <LeaveTypeEdit record={editRecord} onClose={() => setEditRecord(null)} onSuccess={list.invalidate} />
